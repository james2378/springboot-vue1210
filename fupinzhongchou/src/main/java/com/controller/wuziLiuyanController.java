...
public class wuziLiuyanController {
    ...
    */
    ...
    public R update(@RequestBody wuziLiuyanEntity wuziLiuyan, HttpServletRequest request){
...
//            wuziLiuyan.setYonghuId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        //根据字段查询是否有相同数据
        Wrapper<wuziLiuyanEntity> queryWrapper = new EntityWrapper<wuziLiuyanEntity>()
            .eq("id",0)
            ;

        logger.info("sql语句:"+queryWrapper.getSqlSegment());
        wuziLiuyanEntity wuziLiuyanEntity = wuziLiuyanService.selectOne(queryWrapper);
        wuziLiuyan.setUpdateTime(new Date());
        if(wuziLiuyanEntity==null){
